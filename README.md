...
- 장착 아이템은 획득 후 런 종료까지 유지
- 같은 장착 아이템을 다시 먹으면 레벨업
- 최대 레벨: `5`
- 최대 레벨에서 중복 획득 시 점수 대체 보상: `+180`

1. `equip-magnet` (자석)
- 레벨별 흡인 범위: `[120, 170, 220, 280, 340]`
- 레벨별 흡인 속도: `[140, 180, 220, 260, 300]`
- 근처 모든 드랍 아이템(기존/장착 포함) 속도를 플레이어 방향으로 보정

2. `equip-barrier-generator` (베리어 생성기)
- 레벨별 생성 주기(ms): `[5000, 4200, 3500, 2900, 2400]`
- 레벨별 최대 개수: `[1, 1, 2, 2, 3]`
- 베리어는 플레이어 주변 회전형
- 탄막과 충돌 시 베리어+탄막 동시 제거
...
1. `ItemKind`에 신규 장착 타입 추가
2. `EquipmentState`에 레벨 필드 추가
3. `gameConfig.equipment`에 레벨별 수치 추가
4. `EquipmentSystem`에 동작 로직 추가
5. `GameEngine.createItemEffectContext()`에서 `upgradeEquipment` 연결
6. `GameSnapshot`/HUD에 표시 필드 노출
